...
/*   Updated: 2024/01/24 01:43:39 by dcaldas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reduce_b(t_stack **stack_a, t_stack **stack_b)
{
	...
	while (stack_size(*stack_a) > 3 && !is_sorted(*stack_a))
	{
		...
		while (i >= 0)
		{
			...
			else if (i == rrarrb_quantity_b(*stack_a, *stack_b, tmp->n))
	...
	while (*stack_b)
	{
		...
		while (i >= 0)
		{
			if (i == rrarb_quantity_a(*stack_a, *stack_b, tmp->n))
				i = apply_rrarb(stack_a, stack_b, tmp->n, 'b');
			else if (i == rarb_quantity_a(*stack_a, *stack_b, tmp->n))
				i = apply_rarb(stack_a, stack_b, tmp->n, 'b');
			else if (i == rarrb_quantity_a(*stack_a, *stack_b, tmp->n))
				i = apply_rarrb(stack_a, stack_b, tmp->n, 'b');
			else if (i == rrarrb_quantity_a(*stack_a, *stack_b, tmp->n))
				i = apply_rrarrb(stack_a, stack_b, tmp->n, 'b');
			else
				tmp = tmp->next;
		}
	}
